...
export const PDFMergerTool: React.FC = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-500">
      <Card 
        title={t('tool_pdf-merger_name' as any)} 
        description={t('tool_pdf-merger_desc' as any)}
      >
        <div className="space-y-6">
          ...
          <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-10 text-center hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors relative">
            <input 
              ...
              title={t('pdf_click_add' as any)}
            />
            <div className="flex flex-col items-center pointer-events-none">
              <FileUp className="h-10 w-10 text-blue-500 mb-4" />
              <p className="text-lg font-medium text-slate-700 dark:text-slate-200">
                {t('pdf_drag_drop' as any)}
              </p>
              <p className="text-sm text-slate-500 mt-2">{t('pdf_supports' as any)}</p>
            </div>
          </div>
          ...
          {files.length > 0 && (
            <div className="space-y-3">
              <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                {t('pdf_selected' as any)} ({files.length}):
              </h4>
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
            <Button 
              ...
              icon={Layers}
            >
              {isMerging ? t('pdf_processing' as any) : t('pdf_merge_btn' as any)}
            </Button>
            {files.length > 0 && files.length < 2 && (
              <p className="text-xs text-center text-amber-600 mt-2">
                {t('pdf_min_files' as any)}
              </p>
            )}
          </div>
